...
const TableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      render: (text, record) => (
        ...
        <Popconfirm title="Sure to change?" onConfirm={() => handleChange(record)}>
          <a>{record.status === 0 ? '审核' : record.status === 1 ? '正常' : '冻结'}</a>
        </Popconfirm>
      ),
      // ) : null,
    },
    // 在搜索框中显示
    {
      ...
      hideInSearch: false,
      valueEnum: {
        0: { text: '审核', status: 'Default' },
        1: { text: '正常', status: 'Default' },
        2: { text: '冻结', status: 'Processing' },
      },
    },
    {
      title: '身份',
      ...
      // hideInTable:true,
      valueEnum: {
        1: { text: '代理', status: 'Default' },
        0: { text: '会员', status: 'Default' },
      },
    },
    {
      title: '创建人',
  ...
  return (
    <>
      ...
      {stepFormValues && Object.keys(stepFormValues).length ? (
        <UpdateForm
          onSubmit={async (value) => {
            const success = await addRule(value);
            if (success) {
