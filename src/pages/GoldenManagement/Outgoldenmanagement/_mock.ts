...
function postRule(req: Request, res: Response, u: string, b: Request) {
  ...
  const { method, id, zh_name, en_name, exchange_rate, status, key } = body;
  ...
  switch (method) {
    ...
    case 'update':
      tableListDataSource = tableListDataSource.map((item) => {
        if (item.key === key) {
          return { ...item, id, zh_name, en_name,exchange_rate,  status };
        }
        return item;
      });
      break;
...
export default {
  'GET /api/GoldenOut': getRule,
  'POST /api/GoldenOut': postRule,
};
